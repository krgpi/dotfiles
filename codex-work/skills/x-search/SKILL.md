---
name: x-search
description: X(Twitter)で特定トピックをリサーチする。技術トピックに対するコミュニティの反応・評判、最新のトレンドや障害情報、ライブラリ/ツールの実際のユーザー体験を調べたいときに使う。
metadata:
  short-description: X(Twitter)で特定トピックをリサーチする
---

# x-search

X(Twitter)上の情報を検索・収集し、リサーチレポートを作成する。ユーザーの依頼内容から調査したいトピックを読み取って進める。
...
xAI の Responses API に `x_search` ビルトインツールを使わせて、対象トピックに関するX上の投稿をAI分析付きで取得する。`<トピック>` は依頼内容から読み取った検索トピックに置き換える。
...
curl -s https://api.x.ai/v1/responses \
  ...
  -d '{
    ...
    "input": "X上で「<トピック>」について調査してください。主要な意見、注目ツイート、賛否両論を整理してください。"
  }'
```
...
curl -s -G "https://api.twitter.com/2/tweets/search/recent" \
  --data-urlencode "query=<トピック> -is:retweet lang:ja" \
...
Web検索が使える場合は、公式ドキュメント・GitHub・ブログ等の一次情報も検索して補完する。
