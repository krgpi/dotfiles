---
name: mock-ui
description: アプリのUIコードからモックアップ画像（PNG）を生成する。ユーザーが「UIのキャプチャ」「スクリーンショットを作って」「モックアップ画像が欲しい」「LP用の画像」等、アプリを起動せずにUI画像が必要なときに使う。
metadata:
  short-description: UIコードからモックアップ画像を生成する
---
...
ユーザーの依頼内容を生成したいモックアップ画面の説明として解釈する。複数の画面を一度に依頼された場合は、それぞれ個別のHTMLファイルとスクリーンショットを生成する。

## ワークフロー

### Phase 1: デザインシステムの調査

1. アプリのソースコードの場所を特定する（CLAUDE.md/AGENTS.md、package.json、既知のパスから推測）
2. 自分でコードベースを調査し、以下を確認する:
...
2. 各HTMLファイルは以下の要件を満たす:
   ...
   - **リアルなデータ**: ユーザーが指定したテーマに合ったダミーコンテンツを使用。ただし個人情報は安全なダミーにする（後述「ダミー個人情報のルール」参照）
...
1. 作業用ディレクトリに screenshot スクリプト（`.mjs`）を作成する:
...
- **ダミー個人情報のルール**:
  - **名前**: 実在しないことが明らかな架空の名前を使う（例: 空野はるか、星見ゆう、風間そら）。実在しうる平凡な名前（田中太郎など）は避ける
  - **メールアドレス**: RFC 2606 で予約済みのドメインのみ使用する（`example.com`, `example.org`, `example.net`）。例: `haruka@example.com`
  - **電話番号**: 明らかにダミーとわかる番号（例: 000-0000-0000）を使う
  - **住所・その他の個人情報**: 同様に、実在しない架空のものを使う
