---
name: debug-ui
description: UIのスタイリング・レイアウト問題をPlaywrightで診断する。CSSのレイアウト崩れ、レスポンシブ対応の確認が必要なとき、ユーザーが「レイアウトがおかしい」「崩れている」「はみ出す」「スタイルが当たらない」等と言ったときに使う。
metadata:
  short-description: UIのスタイリング問題をPlaywrightで診断する
---

# debug-ui

Playwrightでブラウザを操作し、UIのスタイリング・レイアウト問題を診断する。Chrome拡張のような「今開いているタブ」を扱う仕組みはないため、対象URLをPlaywrightで開いて診断する。

## 引数の解釈

ユーザーの依頼内容を以下のように解釈する:

- **URLの場合** (`http`, `https`, `localhost` で始まる): そのURLをPlaywrightで開いてから診断を開始
- **CSSセレクタの場合** (`.`, `#`, `[` で始まる): 対象URLが不明なら、診断対象のページのURL（開発サーバーのアドレス等）をユーザーに確認してから、そのセレクタの要素を重点的に調査
- **それ以外**: 問題の説明として解釈し、診断対象のURLをユーザーに確認したうえで該当箇所を特定する

## 準備

作業用ディレクトリにPlaywrightのスクリプト（`.mjs`）を作成して進める。Playwrightが未インストールの場合は `npm init -y && npm install playwright && npx playwright install chromium` でセットアップする。以降の各Phaseは、同じスクリプトの中で `page` オブジェクトに対する操作として組み立て、必要な結果を `console.log(JSON.stringify(...))` で出力して確認する。
...
1. **スクリーンショット撮影**
   `page.screenshot({ path: '<name>.png', fullPage: true })` で現在の状態を視覚的に記録し、生成した画像を確認する。

2. **DOM構造の取得**
   `page.content()` またはページ内 `document.body.outerHTML` を `page.evaluate` 経由で取得し、要素階層を把握する。

3. **問題箇所の特定**
   - 引数がCSSセレクタの場合: そのセレクタをそのまま対象にする
   - 問題の説明の場合: スクリーンショットとDOM構造から該当箇所を推定し、対象要素のセレクタを決める

### Phase 2: スタイル詳細分析

対象要素とその親要素に対して `page.evaluate` で以下のスクリプトを実行する。
...
`page.evaluate` で以下のスクリプトを実行し、ページ全体の一般的なCSS問題を検出する:
...
要素数が非常に多いページ（5000以上）では、このスキャンを問題箇所周辺（対象要素の祖先・子孫のみ）に絞る。

### Phase 4: レスポンシブテスト

以下の順に `page.setViewportSize({ width, height })` → `page.screenshot()` を繰り返す:
...
各サイズで視覚的に問題が見つかった場合は、Phase 3 のアンチパターン検出スクリプトを再実行する。テストが終わったら元のビューポートサイズ（1280x800）に戻す。

### Phase 5: ソースコード相関

1. **Reactコンポーネント名の取得**
   `page.evaluate` で以下を実行:
...
3. **ソースファイルの特定**
   コンポーネント名が判明したら:
   - `**/<ComponentName>.tsx`, `**/<ComponentName>.jsx`, `**/<component-name>.tsx` をファイル検索する
   - `function <ComponentName>` や `export.*<ComponentName>` をgrepで検索する
   - 見つかったファイルを読み、問題の原因となるスタイル定義（className, style prop, CSS Modules）を特定する

4. **Hydrationエラーの確認**
   Phase開始時から蓄積している `consoleMessages` の中から `hydrat|mismatch|Warning.*did not match` にマッチするものを探し、Next.js固有のスタイル不一致を確認する。
