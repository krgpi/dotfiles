...
- ウィンドウごとに1プロセス走るが、描画するのは画面に出ているウィンドウのものだけ。内容が変わらないときは描画しないのでちらつかない（裏に回っているあいだも直前の描画内容は捨てない。tmux がペインを保持しているので、戻ってきたときに変化ぶんだけ書けばよい）
...
- **フォルダ名の右に git 状態**: ブランチ名と変更ファイル数（`main *3`）。git 管理下でなければ何も出さない。幅が足りないときはフォルダ名を優先し、ブランチ名から切り詰める
- `git status` を**定期的には回さない**。Claude のステータスや未読が変われば表示が動くので、そのついでに数え直す（＋再描画の SIGUSR1 でも。連続で動くときは3秒の下限を挟む）。アイドル中は1回も走らない代わりに、tmux の外でコミットしても次に何か動くまで数字は古いまま
- 結果は `/tmp/tmux-sidebar-git/<フォルダ>` に置き、描画側は読むだけなので status が遅いリポジトリでもサイドバーは止まらない。**各サイドバーは自分のフォルダのぶんしか数えない**ので、フォルダをまたいだ重複実行にならない（バックグラウンドの子では `trap - EXIT` を外す。親の EXIT トラップが動くとカーソル復帰のエスケープがサイドバーへ流れる）
