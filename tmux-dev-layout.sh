...
# 2. 右側にClaude Code用ペインを作成（幅40%）
tmux split-window -h -p 40 -t $SESSION -c "$WORK_DIR"

# 3. 左側のペイン(0)を上下に分割
tmux select-pane -t dev.0
tmux split-window -v -p 40 -t $SESSION -c "$WORK_DIR"
...
# 左下ペイン（lazygit）
tmux send-keys -t dev.2 'lazygit' C-m
